...
class SceneParsingMITDataset(ThreadedDataset):
    ...
    SceneParse150 [SceneParsing1]_ provides a standard training and
    evaluation platform for scene parsing algorithms. The data for this
    benchmark comes from ADE20K Dataset, which contains more than 20K
    scene-centric images exhaustively annotated with objects and object
    parts. The dataset contains 20K annotated images for training and 2K
    annotated images for validation. The test set is a separate batch of
    images, for which segmentation labels are not provided.  The ground
    truth labels associate each pixel with one of the 150 semantic
    classes, plus one void class.

    The dataset should be downloaded from [SceneParsing1]_ into the
    ...
    References
    ----------
    .. [SceneParsing1] http://sceneparsing.csail.mit.edu/
    '''
    name = 'scene_parsing_MIT'
    non_void_nclasses = 150
    _void_labels = [0]

    GTclasses = list(range(non_void_nclasses)) + _void_labels
